...
export default function ProjectCTA({ links, onNext, slug }) {
  const isGithubRestricted = slug === "bulsuspace";
  ...
  return (
    <div className="px-6 py-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Reveal delay={0} y={14} threshold={0} rootMargin="0px">
          <div className="flex flex-wrap gap-2.5">
            ...
            {isGithubRestricted ? (
              <span
                className="inline-flex cursor-not-allowed items-center gap-2 rounded-[2px] border border-white/15 bg-transparent px-[22px] py-[11px] font-mono text-xs uppercase tracking-[0.1em] text-[#f5f0e8]/40"
                aria-disabled="true"
              >
                GitHub ↗
              </span>
      ...
      {isGithubRestricted && (
        <Reveal delay={140} y={10} threshold={0} rootMargin="0px">
          <p className="mt-3 font-mono text-[11px] tracking-[0.02em] text-[#f5f0e8]/50">
            Source code is private per university data privacy policy.
          </p>
        </Reveal>
      )}
    </div>
